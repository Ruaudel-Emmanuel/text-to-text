...
How it works
A user sends a message through a local chat interface.

The workflow stores the session context.

PostgreSQL provides business rules used to guide the analysis.

The local LLM extracts structured data from the message.

The workflow checks whether important information is missing.

If needed, it asks for clarification.

If the request is complete, it generates an email draft.

The final result is logged into PostgreSQL.
